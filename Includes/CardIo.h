...
public:
	...
	uint8_t G() {
		return static_cast<uint8_t>(status.position);
	}
protected:
	// Status bytes:
	//////////////////////////////////////////////
	...
	struct Status {
		P p = P::NO_ERR;
		S s = S::NO_JOB;
		Position position = Position::NO_CARD;
	...
	virtual uint8_t GetPositionByte() = 0;
	virtual void UpdateState() = 0;

	bool HasCard() {
		return status.position != Position::NO_CARD && status.position != Position::EJECT;
	}
